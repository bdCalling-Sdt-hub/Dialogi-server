...
const successPayment = async (req, res) => {
  try {
    ...
    if (paymentInfo) {
      ...
      const user = await getUserById(req.user._id);
      user.subscription = paymentInfo?.subscription?.type;
      user.subscriptionId = paymentInfo?.subscription?._id;
      await user.save();
    ...
    // update user subscription
    return res.status(200).json(response({ status: 'Success', statusCode: '200', type: 'payment', message: req.t('payment-success'), data: paymentInfo, accessToken: accessToken }));
  } catch (error) {
    console.error(error);
    logger.error(error.message, req.originalUrl);
    return res.status(500).json(response({ status: 'Error', statusCode: '500', type: 'payment', message: req.t('server-error') }));
